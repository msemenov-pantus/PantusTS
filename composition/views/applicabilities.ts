...
export function ApplicabilitiesView() {
  ...
  const ApplicabilitiesVuex = computed(() => {
    return store.getters["applicabilities/applicabilities-index/GetApplicabilities"];
  });
  useFetch(async () => {
    await store.dispatch("applicabilities/applicabilities-index/RequestApplicabilities");
  });
