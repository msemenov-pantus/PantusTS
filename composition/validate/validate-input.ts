export function ValidateInput(FormData:any, NameInput: string) {
    ...
    const ErrorUndefined = (name:string) => {
        FormData[name].error.undefined.active =
        FormData[name].value === "";
    }
    const ErrorRegExp = (name:string) =>{
        FormData[name].error.RegExp.active = !FormData[
          name
        ].params.RegExp.test(FormData[name].value);
    }
    const ErrorValueTrue = (name:string) =>{
        FormData[
            FormData[name].params.valueTrue.valueSet
    ...
    const ErrorValueFalse = (name:string) =>{
        FormData[
            FormData[name].params.valueFalse.valueSet
          ].error.valueTrue.active =
